...
- [ ] 1.1 Add `OutputValueSchema`, `OutputFileSchema`,
...
- [ ] 1.2 Add `outputs: z.record(<keyRegex>, OutputDefSchema).optional()`
...
- [ ] 1.3 Extend the existing node-level accepted-key-set
      validator (used for strict-on-extras) to include `outputs`.
- [ ] 1.4 Confirm `extends:` merging preserves `outputs:` per
...
- [ ] 1.5 Add `OutputDef` and `NodeOutputIndex` types to the
...
- [ ] 3.2 mkdirp the outputs directory (recursive, `0o755`)
      before invoking the executor.
- [ ] 3.3 Thread `outputsDir` through `RunContext` to every
      executor (extend `ctx` per the `executor/types.ts` shape).
- [ ] 3.4 After the executor terminates and the terminal status
...
- [ ] 3.5 When the missing set is non-empty AND the node's
...
- [ ] 3.6 When the node terminated `failed` for any reason
      OTHER than missing-output, skip validation entirely and
      leave `NodeResult.outputs = null`.
- [ ] 3.7 Tests in `src/runner/run.test.ts` covering every
...
- [ ] 5.5 Wire `runner/run.ts` to invoke `recordNodeOutputs`
      after the validation pass and before `recordNodeEnd` for
      the same iteration.
- [ ] 5.6 Tests in `src/storage/sqlite.test.ts` covering:
...
- [ ] 6.1 In `src/cli/runs.ts` (or wherever `runs show` is
      implemented), add the `--outputs` flag to the
      `runs show` subcommand.
- [ ] 6.2 After printing the event log, call
...
- [ ] 7.4 Dispatch by output type:
  ...
  - `directory` with trailing filename → resolve the file
    ...
    contents.
- [ ] 7.5 Exit-code contract per the requirement: `0` on
    success; `1` on usage error, unknown id/node/key/iteration,
    missing-on-disk-file, or fatal I/O.
- [ ] 7.6 Tests covering every scenario in the
      "`minifac runs cat <id> <selector>` subcommand"
      requirement.

## 8. CLI — `prune --outputs`

- [ ] 8.1 Add the `--outputs` flag to the `prune` subcommand in
      `src/cli.ts`. Default off; purely additive.
- [ ] 8.2 In `src/worktree/prune.ts` (or a sibling
...
- [ ] 8.3 When `--outputs` is supplied, walk
...
- [ ] 9.3 Add backlinks to `Outputs.md` from `Factory.md`,
      `Run.md`, and `Runs-DB.md`.
- [ ] 9.4 Mention `docs/decisions/0027-Node-Outputs.md` in
      `Outputs.md`'s prose as the binding architectural
      decision.

## 10. Integration check

- [ ] 10.1 Verify the existing `examples/hello.yaml` and
...
- [ ] 10.2 Add an integration test that defines a tiny two-node
...
- [ ] 10.3 Add an integration test where the writer node
...
- [ ] 10.4 Run the full test suite (`npm test` or whatever
