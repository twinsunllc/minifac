...
- [x] 4.2 Implement the empty-state hint
      ("Press ↑/↓ to select a brief, Enter to drill in") for
      when no brief is selected.
- [x] 4.3 Wire focus routing (`focus = "brief-list"` vs
...
- [x] 4.4 Bind `Esc` (only when `focus = "run-view"`) to
      produce `back-to-list`; bind `r` and `q` at the
      autorun TUI level (not delegated to the embedded
      run-mode view).
- [x] 4.5 Implement the sub-80×24 single-pane collapse per
      the spec.

## 5. Autorun renderer adapter

- [x] 5.1 Create `src/tui/autorun-renderer.ts` exporting
...
- [x] 5.2 `onEvent` dispatches each `AutorunEvent` into
      `autorunReducer` and triggers a re-render.
- [x] 5.3 `onRunEvent(change, entry)` looks up the brief's
      `RunState` slot, runs `runReducer` against it, and
      writes the updated slot back into `BriefListState`.
- [x] 5.4 The `r` hotkey unmounts ink and resolves
...
- [x] 5.5 The `q` hotkey on a no-in-flight state quits
...
- [x] 6.1 In `src/cli/autorun.ts`, after `validateOptions`,
      resolve the output mode via the new flags + helper.
- [x] 6.2 In raw mode, keep today's `makeLogger` /
      scheduler callbacks unchanged.
- [x] 6.3 In TUI mode, construct an `InkAutorunRenderer` and
      ...
      is mounted.
- [x] 6.4 Extend `AutorunRunFactory` (in
...
- [x] 6.5 On `action: "quit"` from the renderer, invoke the
      ...
      process.
- [x] 6.6 Preserve the existing final stderr summary lines
      after ink unmounts so scripts grepping stderr still see
      them.

## 7. CLI mode-selection tests

- [x] 7.1 Add `src/cli.autorun-tui.test.ts` mirroring
      ...
      events).
- [x] 7.2 `process.stdout.isTTY = false` with no flags routes
      the `autorun` action to the raw path (assert today's
      log lines are produced; the renderer is NOT
      constructed).
- [x] 7.3 `--raw` forces raw in a TTY; `--tui` forces TUI in
      a non-TTY.
- [x] 7.4 `--raw --tui` together exits `1` with the flag-
      conflict stderr message and starts no poll cycle.
- [x] 7.5 `--tui --json` together exits `1` with the flag-
      conflict stderr message.
- [x] 7.6 `--json` on a TTY without `--tui` keeps emitting
      JSON on stdout (no TUI is mounted).

## 8. Autorun TUI render snapshot tests

- [x] 8.1 Using `ink-testing-library`, render `<AutorunApp>`
      with an empty `BriefListState` and snapshot the empty-
      state frame.
- [x] 8.2 Snapshot a state with one brief in each status
      (queued, running, succeeded, failed, skipped).
- [x] 8.3 Snapshot the drilled-in state (`focus =
...
- [x] 8.4 Snapshot the sub-80×24 single-pane fallback for
      both focus states.
- [x] 8.5 Snapshot the help overlay (`?`).
- [x] 8.6 Snapshot the mid-drain hotkey bar (after the user
      pressed `q` and at least one run is still in flight).

## 9. Hotkey behavior tests

- [x] 9.1 Test `↓` / `j` and `↑` / `k` move brief-list
      selection without leaving the brief-list focus.
- [x] 9.2 Test `Enter` on a brief with a populated
      `RunState` slot flips focus to `run-view`; `Enter` on
      a queued / skipped brief is a no-op.
- [x] 9.3 Test `Esc` from `run-view` returns to
      `brief-list` and preserves the embedded `RunState`.
- [x] 9.4 Test `r` unmounts ink and resolves the renderer's
      `waitForExit()` with `action: "raw-switch"`.
- [x] 9.5 Test `q` with no in-flight runs quits immediately
...
- [x] 9.6 Test `?` toggles the help overlay.

## 10. Documentation

- [x] 10.1 Update `docs/concepts/Auto-Mode.md` (or create the
      file if it does not exist yet) to mention the TUI is
      the default in a TTY and `--raw` is the escape hatch
      for scripts and CI.
- [x] 10.2 Update `README.md` quickstart to mention the
      autorun TUI alongside the run TUI; call out `--raw` for
      pipelines.

## 11. Acceptance

- [x] 11.1 `minifac autorun` in a TTY shows the autorun TUI.
- [x] 11.2 `minifac autorun > log` (non-TTY) falls back to
      raw output with no behavior change vs today.
- [x] 11.3 `--raw`, `--tui`, and the `--tui` + `--json`
      conflict all behave per the spec.
- [x] 11.4 Selecting a brief drills into the existing run-
      mode TUI for that brief's run; selecting a node within
      it shows its log stream.
- [x] 11.5 Existing `--raw` and `--json` autorun output is
      byte-identical to today.
- [x] 11.6 All existing tests still pass.
- [x] 11.7 `npm run build` is clean.
- [x] 11.8 `openspec validate autorun-tui` exits `0`.
