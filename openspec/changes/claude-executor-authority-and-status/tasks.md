...
- [ ] 1.1 Extend `WithSchema` in `src/executor/claude.ts` with three new
...
- [ ] 1.2 Confirm Zod parse-failure path still produces the existing
      `failed` status with `meta: { reason: "invalid_with", error: ... }`
      for the new fields' validation errors.

## 2. CLI args construction

- [ ] 2.1 Build the `cliArgs` array conditionally based on `with:`
...
- [ ] 2.2 Place the new flags before any pre-existing
      `with.args` passthrough so typed flags lead and `args` trails.
- [ ] 2.3 Verify by inspection that omitting all three fields yields
...
- [ ] 3.5 When no sentinel matches, retain the existing exit-code
      semantics (`0` → `succeeded`, non-zero → `failed`) and continue
      to surface `exitCode` in meta.
- [ ] 3.6 Preserve the existing `missing_binary` path (ENOENT) — that
      branch terminates before any stdout is parsed and is independent
      of the sentinel.

## 4. Documentation in source

- [ ] 4.1 Update the wire-format comment block at the top of
...
- [ ] 4.2 Cross-reference the spec requirement names in the comment so
      future readers can find the canonical contract.

## 5. Tests

- [ ] 5.1 Add a snapshot test for `cliArgs` with `{ prompt: "hi" }`
      that locks the pre-change argv exactly (backwards-compat guard).
- [ ] 5.2 Add a snapshot test for `cliArgs` with all three new fields
      set (e.g. `permission_mode: "accept_edits"`,
      `allowed_tools: ["Bash(openspec:*)", "Write"]`,
      `add_dirs: ["/tmp/x", "/tmp/y"]`) covering ordering and joining.
- [ ] 5.3 Test: invalid `permission_mode` value → terminal `failed`
      status with `meta.reason === "invalid_with"`, no child spawn.
- [ ] 5.4 Test: empty-string element in `allowed_tools` → terminal
      `failed` status with `meta.reason === "invalid_with"`, no child
      spawn.
- [ ] 5.5 Test: empty-string element in `add_dirs` → terminal `failed`
      status with `meta.reason === "invalid_with"`, no child spawn.
- [ ] 5.6 Test: child exits `0`, final `result.result` ends with
...
- [ ] 6.3 Run `openspec validate claude-executor-authority-and-status`
      and confirm clean.
