...
"Step inlining order" and "Step input validation" requirements), the
per-node outputs directory (per the "Per-node-per-iteration outputs
directory" requirement), and the run-wide `priorResults` snapshot
(per the "Prior-results accumulate" requirement), these form the
...
1. **Simple namespace token:** literal `{{`, optional ASCII
   ...
   `[a-zA-Z_][a-zA-Z0-9_]*`, an optional dotted sub-path (zero or
   more of literal `.` and an identifier of the same form), optional
   ASCII whitespace, literal `}}`. A sub-path resolves only under
   `run.split` and `run.split_integration` (see "Split run-context
   tokens and preamble block"); any other token carrying one passes
   through verbatim.
...
scope (see above). When the run has no `runBaseBranch` in scope,
`run.base_branch` tokens SHALL be left verbatim (matching
